...
const TodoApp: React.FC = () => {
  ...
  // Create task on backend and update frontend
          ...
          <div className="p-6 bg-slate-50/50 border-b border-slate-200/60">
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-200/60">
              <div className="flex gap-8 text-sm">
                <span className="text-slate-600"><span className="font-semibold text-slate-800">{todoStats.total}</span> Total</span>
                <span className="text-slate-600"><span className="font-semibold text-amber-600">{todoStats.active}</span> Active</span>
                <span className="text-slate-600"><span className="font-semibold text-emerald-600">{todoStats.completed}</span> Completed</span>
              </div>
          ...
          <div className="p-6">
            {filteredTodos.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-xl font-light text-slate-500 mb-2">{emptyState.title}</p>
                {emptyState.subtitle && <p className="text-slate-400">{emptyState.subtitle}</p>}
              </div>
            ) : (
              <div className="space-y-3">
                {filteredTodos.map((todo: Todo) => (
                  <div
                    key={todo.id}
                    className={`group relative bg-white/60 backdrop-blur-sm border border-slate-200/60 rounded-xl p-5 transition-all duration-200 hover:shadow-md hover:bg-white/80 ${todo.completed ? 'opacity-75' : ''}`}
                  >
                    <div className="flex items-start gap-4">
                      <button
                        ...
                        className={`flex-shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all duration-200 mt-0.5 ${
                          todo.completed ? 'bg-emerald-500 border-emerald-500 text-white shadow-md' : 'border-slate-300 hover:border-emerald-400 hover:bg-emerald-50'
                        }`}
                        ...
                        {editingId === todo.id ? (
                          <div className="flex gap-3">
                            ...
                            <div className="flex gap-2">
                              <button onClick={saveEdit} className="text-emerald-600 hover:text-emerald-700 p-1 rounded transition-colors" aria-label="Save changes">
                                <Check size={18} />
                              </button>
                              <button onClick={cancelEdit} className="text-slate-400 hover:text-slate-600 p-1 rounded transition-colors" aria-label="Cancel editing">
                                <X size={18} />
                              </button>
                            </div>
                          </div>
                          ...
                          <div>
                            <p className={`text-lg leading-relaxed ${todo.completed ? 'line-through text-slate-400' : 'text-slate-700'}`}>{todo.text}</p>
                            <p className="text-sm text-slate-400 mt-2 font-light">{todo.createdAt}</p>
                          </div>
                        )}
                      </div>
                      ...
                      {editingId !== todo.id && (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-2">
                          <button onClick={() => startEdit(todo.id, todo.text)} className="text-slate-400 hover:text-indigo-600 p-2 rounded-lg hover:bg-indigo-50 transition-all duration-200" aria-label="Edit task">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => deleteTodo(todo.id)} className="text-slate-400 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 transition-all duration-200" aria-label="Delete task">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      )}
